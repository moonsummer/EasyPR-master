...
#include "include/easypr/network/server_socket.hpp"
		...
		while (1){
			if (isIdentify)	{
				...
				//	imshow("preview", image);
					...
					//  waitKey(100);
					cout << "test plate recognize\n" << endl;
					usleep(100000);
					assert(test_plate_recognize(image) == 0);
					if (waitKey(10) >= 0)
						isIdentify = true;
				}
			}
...
int main(int argc, const char* argv[]) {
	struct socket_config skcfg;
    strcpy(skcfg.local_ip, "192.168.3.223");
    strcpy(skcfg.multicast_addr, "255.255.255.255");
    skcfg.multicast_port = 20000;
    skcfg.server_port = 20001;
    if (server_socket_init(skcfg))
    {   
